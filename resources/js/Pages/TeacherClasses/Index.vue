    ...
    <AuthenticatedLayout>
        <div class="container mx-auto px-4 py-8">
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-4xl font-extrabold text-gray-900 tracking-tight leading-tight">Teacher Classes</h1>
                    ...
                    <table class="min-w-full bg-white rounded-lg border-collapse">
                        <thead class="bg-gradient-to-r from-blue-500 to-blue-700 text-white uppercase text-sm font-semibold">
                            <tr>
                                ...
                                <th class="w-1/6 py-4 px-6 text-left">Teacher</th>
                            </tr>
                        </thead>
                        <tbody class="text-gray-700 text-sm">
                            <tr v-for="(classItem, index) in classLists" :key="index" class="border-b border-gray-200 hover:bg-gray-50 transition duration-150 ease-in-out">
                                ...
                                <td class="py-4 px-6">{{ getTimeForClass(classItem, index) }}</td>
                                <td class="py-4 px-6">
                                ...
                                <td class="py-4 px-6">{{ classItem.teacher?.name || 'No Teacher' }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    export default {
        props: {
            classes: Array, // Classes data passed from the backend
        },
        data() {
            return {
                currentDate: '',
        ...
        computed: {
            weekdays() {
                // Returning an object grouped by day names with their respective classes
        ...
        methods: {
            ...
            getTimeForClass(classItem, index) {
                return this.lessonTimes[index] || 'Time not available';
            },
            prevWeek() {
            ...
            loadClassesForWeek() {
                ...
                this.$inertia.get('/TeacherClasses', {
                    ...
                    console.error('Error fetching classes for the week:', error);
                });
            },
            groupClassesByDay() {
                ...
                this.classes.forEach((classItem) => {
                    if (!classItem.Date) {
                        console.error('Invalid or missing Date for classItem:', classItem);
                        return; // Skip classItems without valid dates
                    }

                    const date = parseISO(classItem.Date); // Ensure the date is ISO format
                    ...
                    switch (dayOfWeek) {
                        ...
                        default:
                            console.error('Unexpected day:', dayOfWeek);
                    }
                });
